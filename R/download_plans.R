...
for(i in 1:nrow(plan_urls)){
   temp <- plan_urls[i,]
   if(is.na(temp$url_dl))next()
   if(is.na(year.ind)){year.ind="NaN"}

   fn <- paste0(temp$year_start, "_",temp$country,"_", temp$state, "_", temp$plan_type,".", temp$filetype)

   if (!overwrite.plans &
       fn %in% list.files(dir)) {
       message(fn,
               "already exists and overwrite.plans=FALSE. not downloading this file.")
   }else(download.file(url = temp$url_dl,
                                         destfile = paste(dir,fn),
                                         mode = dl.mode))
}

message("for a list of downloaded files, please call `list.files(", paste(dir),")`", " or browse this directory")
